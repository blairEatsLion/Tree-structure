...
public class WordTree
{
	...
	public void insert(String word)
	{
		...
		WordTreeNode getPrefix = getPrefixNode(word);
		
		if(getPrefix == root){
			char firstChar = word.charAt(0);
			getPrefix = getPrefix.createChild(firstChar);
		}	
		
		for(int index=getPrefix.depth;index<word.length();index++){
			
			char addChar = word.charAt(index);			
			getPrefix = getPrefix.createChild(addChar);
			
		}

		getPrefix.endOfWord = true;
	...
	WordTreeNode getPrefixNode(String word)
	{
		...
		for(int i=0; i<word.length();i++){
			...
			if(current == root){
				if(current.children[(int)first] != null){
					current = current.children[(int)first];
				}else{
					return root;
				}
			}else{
				if(current.children[(int)cur] != null){
					current = current.children[(int)cur];
				}else{
					return current;
				}
			}
	...
	{  
		...
		if(word.equals(getPrefix(word))){
			if(getPrefixNode(word).endOfWord == true){
				containing = true;	
			}
		}	
		
		if(getPrefixNode(word) == root){
			containing = false;
		}
		
		//only complete words get true
	...
	public ArrayList<String> getListPrefixMatches( String prefix )
	{
		...
		if(prefix.equals(getPrefix(prefix))){
		//use helper method (written at the end)
			helper(getPrefixNode(prefix),matches);
	...
	public void helper(WordTreeNode cur,ArrayList<String> a)  //helper for getListPrefixMatches()
	{
		WordTreeNode current = cur;
